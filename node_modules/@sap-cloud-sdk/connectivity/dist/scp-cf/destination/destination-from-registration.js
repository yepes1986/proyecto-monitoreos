...
function getJwtForCaching(options) {
    ...
    if (!jwt?.zid) {
        if (options?.jwt) {
            logger.error('Could not determine tenant from JWT nor XSUAA, identity or destination service binding. Destination is registered without tenant information.');
        }
        else {
